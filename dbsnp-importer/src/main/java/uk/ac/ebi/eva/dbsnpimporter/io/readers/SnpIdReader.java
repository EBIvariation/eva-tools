 ...
 */
public class SnpIdReader extends JdbcPagingItemReader<Long> {

    public SnpIdReader(String assembly, List<String> assemblyType, DataSource dataSource, int pageSize)
            throws Exception {
        if (pageSize < 1) {
        ...
        setRowMapper(new SnpIdRowMapper());
...
//        setParameterValues(parameterValues);
    ...
    private PagingQueryProvider createQueryProvider(DataSource dataSource) throws Exception {
        ...
        factoryBean.setSelectClause(
                "SELECT sub.subsnp_id AS ss_id"
        );
        factoryBean.setFromClause(
                "FROM "
                        + "snpsubsnplink link JOIN "
                        + "subsnp sub ON link.subsnp_id = sub.subsnp_id"
        );
